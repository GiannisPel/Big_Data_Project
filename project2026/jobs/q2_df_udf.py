...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Q2 duration/speed/congestion analysis using DataFrame API with Python UDF."
    )
...
#Haversine formula using Spark builtin functions
    ...
    #d = R * c
def haversine_km_python(
    pickup_lat: float,
    pickup_lon: float,
    dropoff_lat: float,
    dropoff_lon: float,
) -> float | None:
    #Return None if coordinate is missing
    ...
    lon1 = math.radians(float(pickup_lon))
    lat2 = math.radians(float(dropoff_lat))
    lon2 = math.radians(float(dropoff_lon))
    ...
    #Clamp to [0, 1] to avoid floating point errors in sqrt/atan2
    a = min(1.0, max(0.0, a))
...
#Make Python function as a Spark UDF returning a double
haversine_udf = F.udf(haversine_km_python, DoubleType())
...
def filter_nyc_coords(df: DataFrame) -> DataFrame:
    #Keep only rows where both pickup AND dropoff are inside NYC bounding box
    return df.filter(
...
def prepare_trips(df: DataFrame, hours: list[int]) -> DataFrame:
    ...
    #Step 4: compute Haversine distance using the Python UDF
    with_haversine = (
        clean
        .withColumn(
            "haversine_km",
    ...
    return (
        ...
        #Gap between odometer distance and straight line Haversine distance
...
def aggregate_by_hour(trips: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .agg(
            ...
            #Aggregate speed = total km / total hours ( by distance, no per trip average)
...
def main() -> None:
    ...
    with Timer() as timer:
        ...
        #Apply all filters and compute derived columns and cache cause query it multiple times
        ...
        #Output paths
        by_hour_path = join_hdfs_path(args.output_base, "results", "tables", "q2", "df_udf_by_hour")
        slowest_path = join_hdfs_path(args.output_base, "results", "tables", "q2", "df_udf_slowest_per_km")
        fastest_path = join_hdfs_path(args.output_base, "results", "tables", "q2", "df_udf_fastest")
        metrics_path = join_hdfs_path(args.output_base, "results", "metrics", "q2_df_udf_metrics.json")
        plan_path    = join_hdfs_path(args.output_base, "results", "plans",   "q2_df_udf_plan.txt")
        ...
        #Save the query plan for report
        plan_text = by_hour._jdf.queryExecution().toString()
        write_text_hdfs(spark, plan_path, plan_text, overwrite=True)

        #Build a metrics dict with everything to save in the report
        metrics = {
            "job": "q2_df_udf",
            "implementation": "DataFrame API with Python UDF",
